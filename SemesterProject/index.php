...
<head>
    ...
    <script type="text/javascript">
        function validateForm() {
            var wName = document.forms["createForm"]["createName"].value;
            ...
            if (wName === "") {
                message = "Please enter a name for the new world.";
                newItem.innerHTML = message;
                newItem.style.backgroundColor = message === "Success" ? "lightgreen" : "red";
...
<body>
    ...
    <table id="worlds">
        <tr>
            <th><button onclick="sortFunction(0)">World Name</button></th>
            <th><button onclick="sortFunction(1)">Owner</button></th>
        </tr>

        <?php
        $c = connect();
        foreach (getAllFromTableWhereUsername($c, "worlds") as $row) {
            echo "<tr>";

            $keys = array("wName", "username");
            foreach ($keys as $key) {
    ...
    <form id="createForm" action="dbUpdate.php" onsubmit="return validateForm()" method="post">
        <label for="createName">Create New World (DO NOT USE SPACES):</label><br>
        <input type="text" name="createName" id="createName">
        <input type="hidden" name="hidden" value="world">
        <br><br>
