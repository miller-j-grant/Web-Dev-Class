...
<head>
    ...
    <script type="text/javascript">
        function validateForm() {
            ...
            if (cName === "") {
                message = "Please enter a name for the new npc.";
                newItem.innerHTML = message;
                newItem.style.backgroundColor = message === "Success" ? "lightgreen" : "red";
        ...
        function sortFunction(column){
            var rows = document.getElementById("npcs").getElementsByTagName("tr");
            var sortValues = [];
...
<body>
    ...
    <table id="npcs">
        <tr>
            <th><button onclick="sortFunction(0)">NPC Name</button></th>
            <th><button onclick="sortFunction(1)">Location Name</button></th>
            <th><button onclick="sortFunction(2)">Organization Name</button></th>
        ...
        <?php
        $c = connect();
        foreach (getAllFromTableWhereUsername($c, "npcs") as $row) {
            echo "<tr>";

            $keys = array("cName", "lName", "oName", "wName", "username");
            foreach ($keys as $key) {
    ...
    <form name="createForm" action="dbUpdate.php" onsubmit="return validateForm()" method="post">
        ...
        <input type="hidden" name="hidden" value="npc">
        <br><br>
